...
# Search for "ENHACEMENT" in this script to make it work
# Get token from Firefox storage (via inspector): MMAUTHTOKEN
# Select team by idx: enter number to corresponding team (list is given)
# Select conversation. List is given.
import os
import sqlite3
...
def select_channel(d: Driver, team: str, my_user_id: str, user_id_to_name: Dict[str, str],
    ...
    for channel in channels:
        ...
        print("Name: " + channel["name"]) # DEBUG
        user_ids = channel["name"].split("__")
        other_user_id = user_ids[1] if user_ids[0] == my_user_id else user_ids[0]
        print("Id: " + other_user_id) # DEBUG
        if other_user_id in user_id_to_name: # ENHACEMENT: Needs to be in if clause
            print("Id IN: " + other_user_id) # DEBUG
...
def export_channel(d: Driver, channel: str, user_id_to_name: Dict[str, str], output_base: str,
    ...
    output = {
        "channel": {
            ...
            "id": channel["id"],
            # "team": d.teams.get_team(channel["team_id"])["name"], # ENHACEMENT: Needs to be commented. Breaks.
